...
final class RenduController extends AbstractController
{
    ...
    {
        $rendu = new Rendu();
        $form = $this->createForm(RenduForm::class, $rendu);
    ...
    #[Route('/etape/{id}/rendu', name: 'app_upload_rendu', methods: ['POST'])]
    public function uploadRendu(Request $request, Etape $etape, EntityManagerInterface $em)
    {
        $file = $request->files->get('document');

        if ($file) {
            $filename = uniqid() . '.' . $file->guessExtension();
            $file->move($this->getParameter('rendu'), $filename);

            $rendu = new Rendu();
            $rendu->setUser($this->getUser());
            $rendu->addEtape($etape);
            $rendu->setUrl($filename);
